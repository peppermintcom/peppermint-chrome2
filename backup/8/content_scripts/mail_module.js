...
--frontier--",
			...
			reply_to_message: function ( receiver, audio_data, response, callback ) {
				...
				gapi.client.gmail.users.messages.send({
					...
					mail_sender.sending = false;
					callback();
				});
			
			},
			reply_mail: function ( audio_data, mail_data, callback ) {
			...
			compose_mail: function ( audio_data, mail_data, callback ) {
				...
				gapi.client.gmail.users.messages.send({
					'userId': 'me',
					'resource': {
						'raw': mail_sender.get_raw_mail( mail_data.receiver, 'Peppermint Voice Mail', '', '', filename, audio_data )
					}
				}).execute(function(response){
					console.log( response );
					mail_sender.sending = false;
		...
		v.add({
			...
			'mail_is_sending': function ( data ) {
				data.callback( mail_sender.sending );
			},
			...
			'gmail_api_ready': function () {
				v.fire({ name: 'check_auth', callback: function ( result ) {
					v.fire({ name: 'check_auth_result', result: result, callback: function ( valid ) {
						if ( valid ) {
							v.fire({ name: 'set_storage', key: 'authorized', value: true });
						} else {
							v.fire({ name: 'set_storage', key: 'authorized', value: false });
						}
					} });
				} });
			},
			...
			'send_data_request': function ( data ) {
				if ( data.reply ) {
					mail_sender.reply_mail( data.audio_data, data.mail_data, data.callback );
				} else if ( data.compose ) {
					mail_sender.compose_mail( data.audio_data, data.mail_data, data.callback );
				}
			},
			'authorize_request': function ( data ) {
				v.fire({ name: 'get_storage', key: 'authorized', callback: function ( authorized ) {
					if ( authorized ) {
						data.callback( true );
					} else {
